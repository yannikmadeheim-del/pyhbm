...
def _load_references(specs):
    """Load the dense reference NFRC CSV for every k_rel present in the specs.

    Returns {k_rel: ReferenceCurve}, or None if any CSV is missing (with a hint).
    """
    k_rels = sorted({params["k_rel"] for (_, _, params, _) in specs.values()})
    curves, missing = {}, []
    for k_rel in k_rels:
        path = refmod.reference_path(k_rel)
        if path.exists():
            curves[k_rel] = refmod.load_reference_csv(path)
        else:
            missing.append(path)
    if missing:
        print("Missing reference CSV(s):")
        for p in missing:
            print("   ", p.name)
        print("\nGenerate them first:  python make_reference.py")
        return None
    return curves
...
def main():
    ...
    print(f"Running {len(specs)} variant(s); trajectory error vs "
          f"{cfg.REFERENCE_LABEL} (per k_rel)\n")
    ...
    plotting.plot_frc_overview(
        results, here / "rod_method_comparison_frc.png",
        om_lin=om_lin, peak_lin=peak_lin, gap=cfg.GAP, reference=ref_curves)
    plotting.plot_metrics(results, here / "rod_method_comparison_metrics.png")
