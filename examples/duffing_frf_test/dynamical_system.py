...
class DuffingForced(FirstOrderODE):
	...
	udot = omega u' = v
	vdot = omega v' = -k*u -c*v - beta*(u**3) + P*cos(tau)
	...
	def nonlinear_term(self, state: np.ndarray, adimensional_time: np.ndarray) -> np.ndarray:
		...
		fnl = -self.beta * np.power(u, 3) #* array(cos(adimensional_time))[...,np.newaxis,np.newaxis]
	...
	def jacobian_nonlinear_term(self, state: np.ndarray, adimensional_time: np.ndarray) -> np.ndarray:
		...
		u = state[..., 0:1, :]  # Select first element along the second-to-last axis
		zeros = np.zeros_like(u)
		dfnldx = -3 * self.beta * np.power(u, 2) #* array(cos(adimensional_time))[...,np.newaxis,np.newaxis]  # Correct coefficient for cubic nonlinearity
