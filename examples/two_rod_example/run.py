...
class BranchResult:
    ...
    # trajectory relative error vs the stored reference, per signal
    # (filled by plotting.compute_trajectory_error)
    traj_err:  dict = field(default_factory=dict)   # {signal: per-point rel. error}
    traj_mean: dict = field(default_factory=dict)
    traj_max:  dict = field(default_factory=dict)
    traj_rms:  dict = field(default_factory=dict)
